...
- Why `0.01s` and not `0` or a negative value: CSS rejects negative durations, so a `-1s` rule would be discarded and every site would keep its own full-length transitions. A `0s` transition never starts, so its events never fire and menus or modals that wait for them can hang. `0.01s` is shorter than one frame, so it looks instant and keeps the events. (Negative values are valid only for *delays*, which DNA Shield sets to `0`.)
...
- On Chromium: injects declarative **Speculation Rules** — the browser itself prefetches a link's HTML after a 10 ms hover (`eager`) and fully prerenders it after 200 ms or on `pointerdown` (`moderate`), reusing the prefetch. Navigating feels like the page was already open, because it was. Sites that enforce Trusted Types (Gmail, Outlook, other Google/Microsoft apps) are handled with full CSP hygiene: the text routes through the site's own default policy where one exists, through a DNA Shield policy where creation is allowed, and is skipped entirely — silently, with zero console noise — where nothing is allowed. On these browsers DNA Shield steps out of prefetching entirely — one owner per engine, no double downloads, no duplicate console noise.
...
3. **CSS:** while a rendered captcha widget is on the page (including a Turnstile widget hidden in a closed shadow root, detected through its `cf-turnstile-response` field), the entire accelerator stylesheet is swapped for a captcha-safe subset (instant scrolling only) and restored the moment it's gone — because slider puzzles are commonly driven by negative `animation-delay` techniques that a universal clamp would destroy. Only widgets a visitor actually faces count: loader and bot-detection scripts, the reCAPTCHA v3 badge, and its hidden challenge frame ship on every page of many sites and never switch acceleration off. Detection only inspects newly added elements, and the single full scan runs in idle time, so it costs the page almost nothing.
